...
function IconBars() {
  return <svg width="14" height="14" viewBox="0 0 24 24" fill="none"><path d="M4 19V5M4 19h16M8 15v-4M12 15V8M16 15v-6" stroke="var(--surface-3)" strokeWidth="2" strokeLinecap="round"/></svg>
}
function IconChecklist() {
  return <svg width="14" height="14" viewBox="0 0 24 24" fill="none"><path d="M9 11l2 2 4-4M5 4h14v16H5z" stroke="var(--surface-3)" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round"/></svg>
}
function IconPlus() {
  ...
  return (
    <button
      ...
      style={{
        ...
        background: saving ? 'var(--ink-3)' : 'var(--brand)',
        color: 'var(--on-brand)',
        border: 'none',
        fontSize: '12px',
        fontWeight: 600,
    ...
    <div style={{ ...card, marginBottom: '10px' }}>
      <div style={cardHeader}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: '11px', color: 'var(--ink-25)' }}>
        ...
        {block.block_type === 'custom' && (
          <button
            ...
            style={{ border: 'none', background: 'transparent', cursor: 'pointer', color: 'var(--line-strong)', padding: '4px', borderRadius: '6px' }}
            title="Удалить блок"
          >
      ...
      {/* Строки тиров */}
      {block.tiers.map(t => (
        <div key={t._id} style={{ display: 'flex', gap: '8px', alignItems: 'center', padding: '4px 14px', borderTop: '1px solid var(--paper-2)' }}>
          <input
            type="number"
            value={t.tier_from}
      ...
      {block.tiers.length === 0 && (
        <div style={{ padding: '12px 14px', color: 'var(--ink-3)', fontSize: '12.5px' }}>Нет ступеней — нажмите «+ Добавить»</div>
      )}
...
export function KpiSettingsPanel() {
  ...
  if (roles.length === 0) {
    return <div style={{ ...card, padding: '20px', color: 'var(--ink-3)', fontSize: '13px' }}>Загрузка ролей…</div>
  }

  return (
    <div style={{ fontFamily: "'IBM Plex Sans', system-ui, sans-serif", color: 'var(--ink)' }}>

      {/* ── Заголовок ── */}
      <div style={{
        background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '18px', padding: '14px 18px',
        display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '10px',
        boxShadow: '0 1px 2px rgba(28,20,22,0.04)',
      }}>
        <div style={{ width: '38px', height: '38px', borderRadius: '10px', background: 'var(--brand)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="9" stroke="var(--surface-3)" strokeWidth="1.6"/><circle cx="12" cy="12" r="5" stroke="rgba(255,255,255,0.62)" strokeWidth="1.6"/><circle cx="12" cy="12" r="1.6" fill="var(--surface-3)"/></svg>
        </div>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: '15px', fontWeight: 600, color: 'var(--ink)' }}>Настройка KPI</div>
          <div style={{ fontSize: '11px', color: 'var(--ink-3)' }}>Мотивация и премии · {selectedRoleLabel}</div>
        </div>
      </div>
      ...
      <div style={{ ...card, display: 'flex', alignItems: 'center', gap: '12px', padding: '11px 14px', marginBottom: '10px', flexWrap: 'wrap' }}>
        <div style={{ flex: 1, minWidth: '140px' }}>
          <div style={{ fontSize: '10px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.7px', color: 'var(--brand)' }}>Роль</div>
          <div style={{ fontSize: '11px', color: 'var(--ink-25)' }}>Настройки ниже относятся к этой роли</div>
        </div>
        <div style={{ position: 'relative', minWidth: '200px' }}>
          <select
            value={selectedRole}
            onChange={e => setSelectedRole(e.target.value)}
            style={{ width: '100%', appearance: 'none', background: 'var(--paper-2)', border: '1.5px solid var(--brand)', borderRadius: '9px', padding: '9px 34px 9px 11px', fontSize: '13.5px', fontWeight: 600, color: 'var(--ink)', cursor: 'pointer', fontFamily: 'inherit' }}
          >
          ...
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" style={{ position: 'absolute', right: '11px', top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none' }}>
            <path d="M5 9l7 7 7-7" stroke="var(--brand)" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        </div>
        {/* Рабочие дни */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '7px', background: 'var(--paper)', border: '1px solid var(--line-mid)', borderRadius: '9px', padding: '7px 11px' }}>
          <span style={{ fontSize: '11.5px', color: 'var(--ink-2)', whiteSpace: 'nowrap' }}>Раб. дней:</span>
          <input
            type="number"
            value={workDaysStr}
            onChange={e => setWorkDaysStr(e.target.value)}
            style={{ width: '44px', border: '1px solid var(--line-mid)', borderRadius: '6px', padding: '4px 5px', fontFamily: "'IBM Plex Mono', monospace", fontSize: '13px', fontWeight: 600, color: 'var(--ink)', textAlign: 'center', background: 'var(--surface)' }}
          />
        </div>
      </div>

      {loading ? (
        <div style={{ padding: '28px', color: 'var(--ink-3)', fontSize: '13px', textAlign: 'center' }}>Загрузка настроек…</div>
      ) : (
        <>
          {/* ── Оклад ── */}
          <div style={{ ...card, marginBottom: '10px' }}>
            <div style={cardHeader}>
              <div style={iconBox}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none"><rect x="3" y="6" width="18" height="12" rx="2" stroke="var(--surface-3)" strokeWidth="1.7"/><circle cx="12" cy="12" r="2.4" stroke="var(--surface-3)" strokeWidth="1.7"/></svg>
              </div>
              <div>
                <div style={{ fontSize: '13px', fontWeight: 600 }}>Оклад</div>
                <div style={{ fontSize: '11px', color: 'var(--ink-25)' }}>Ручной ввод · тип оплаты выбирается ниже</div>
              </div>
            </div>
            <div style={{ padding: '12px 14px' }}>
                ...
                {(['fixed', 'per_shift'] as const).map(type => (
                  <button
                    ...
                    style={{
                      ...
                      padding: '9px 6px',
                      borderRadius: '9px',
                      border: salaryType === type ? '2px solid var(--brand)' : '1.5px solid var(--line-mid)',
                      background: salaryType === type ? 'var(--paper)' : 'var(--paper-2)',
                      cursor: 'pointer',
                      fontFamily: 'inherit',
                    }}
                  >
                    <div style={{ fontSize: '12.5px', fontWeight: 600, color: salaryType === type ? 'var(--brand)' : 'var(--ink-2)' }}>
              ...
              {salaryType === 'fixed' ? (
                <div>
                  <div style={{ fontSize: '11px', color: 'var(--ink-25)', marginBottom: '5px' }}>Сумма в месяц, сом</div>
                  <input
                    type="number"
                    value={salaryAmount}
                ...
                <div style={{ display: 'flex', gap: '10px', alignItems: 'flex-end' }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: '11px', color: 'var(--ink-25)', marginBottom: '5px' }}>Ставка за один выход, сом</div>
                    <input
                      type="number"
                      value={ratePerShift}
                  ...
                  <div style={{ paddingBottom: '10px', color: 'var(--ink-25)', fontSize: '13px' }}>
                    × {workDays || '?'} дн. = <strong style={{ color: 'var(--brand)', fontFamily: "'IBM Plex Mono', monospace" }}>{fmt((Number(ratePerShift) || 0) * workDays)}</strong>
                  </div>
                </div>
              )}
            </div>
            ...
            {showAddBlock ? (
              <div style={{ ...card, padding: '14px', marginBottom: '10px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px 80px', gap: '8px', marginBottom: '10px' }}>
                  <div>
                    <div style={{ fontSize: '11px', color: 'var(--ink-25)', marginBottom: '4px' }}>Название блока *</div>
                    <input
                      type="text"
                      value={newBlockName}
                      onChange={e => setNewBlockName(e.target.value)}
                      placeholder="Например: Бонус за конверсию"
                      style={{ width: '100%', border: '1px solid var(--line)', borderRadius: '7px', padding: '8px 10px', fontSize: '13px', fontFamily: 'inherit', color: 'var(--ink)', background: 'var(--paper-2)', boxSizing: 'border-box' }}
                    />
                  </div>
                  <div>
                    <div style={{ fontSize: '11px', color: 'var(--ink-25)', marginBottom: '4px' }}>Подпись «От»</div>
                    <input
                      type="text"
                  ...
                  <div>
                    <div style={{ fontSize: '11px', color: 'var(--ink-25)', marginBottom: '4px' }}>Подпись «До»</div>
                    <input
                      type="text"
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button
                    ...
                    style={{ background: 'var(--brand)', color: 'var(--on-brand)', border: 'none', borderRadius: '8px', padding: '8px 16px', fontSize: '12.5px', fontWeight: 600, cursor: addingBlock || !newBlockName.trim() ? 'default' : 'pointer', opacity: addingBlock || !newBlockName.trim() ? 0.6 : 1 }}
                  >
                    {addingBlock ? 'Создание…' : 'Создать блок'}
                  </button>
                  <button
                    onClick={() => setShowAddBlock(false)}
                    style={{ background: 'transparent', color: 'var(--ink-25)', border: '1px solid var(--line-mid)', borderRadius: '8px', padding: '8px 14px', fontSize: '12.5px', cursor: 'pointer' }}
                  >
                    Отмена
                  </button>
                </div>
              ...
              <button
                ...
                style={{ width: '100%', border: '1.5px dashed var(--ink-3)', background: 'transparent', color: 'var(--brand)', fontSize: '13px', fontWeight: 600, padding: '12px', borderRadius: '10px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', marginBottom: '10px' }}
              >
          ...
          <div style={{ ...card, marginBottom: '10px' }}>
            <div style={cardHeader}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: '11px', color: 'var(--ink-25)' }}>Именованные задачи с фиксированным бонусом</div>
              </div>
              <button
                onClick={addKpiItem}
                style={{ border: 'none', background: 'var(--brand)', color: 'var(--on-brand)', fontSize: '12px', fontWeight: 600, padding: '7px 12px', borderRadius: '8px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '5px' }}
              >
                <IconPlus /> Добавить
              </button>
            </div>
            ...
            {kpiItems.map(k => (
              <div key={k._id} style={{ display: 'flex', gap: '8px', alignItems: 'center', padding: '5px 14px', borderTop: '1px solid var(--paper-2)' }}>
                <input
                  type="text"
                  value={k.name}
                  placeholder="Название"
                  onChange={e => updateKpiItem(k._id, 'name', e.target.value)}
                  style={{ width: '200px', border: '1px solid var(--line)', borderRadius: '6px', padding: '7px 9px', fontSize: '13px', fontWeight: 500, color: 'var(--ink)', background: 'var(--paper-2)', fontFamily: 'inherit' }}
                />
                <input
                  type="text"
                  value={k.description}
                  ...
                  style={{ flex: 1, minWidth: 0, border: '1px solid var(--line)', borderRadius: '6px', padding: '7px 9px', fontSize: '12.5px', color: 'var(--ink-2)', background: 'var(--paper-2)', fontFamily: 'inherit' }}
                />
                <input
                  type="number"
            ...
            {kpiItems.length === 0 && (
              <div style={{ padding: '14px', color: 'var(--ink-3)', fontSize: '12.5px', textAlign: 'center' }}>Нет пунктов</div>
            )}
          ...
          {/* ── Зарплата в идеале ── */}
          <div style={{ background: 'var(--paper)', border: '1px solid var(--line)', borderRadius: '18px', padding: '14px 18px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="9" stroke="var(--brand)" strokeWidth="2"/><path d="M9.5 9.5h3.2a1.8 1.8 0 010 3.6H10m0 0h3a1.8 1.8 0 010 3.6H9.5M11 7v10.5" stroke="rgba(255,255,255,0.72)" strokeWidth="1.4" strokeLinecap="round"/></svg>
              <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--ink)' }}>Зарплата в идеале</span>
              <span style={{ fontSize: '11px', color: 'var(--ink-3)' }}>При максимальных показателях и всех KPI</span>
            </div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', alignItems: 'center' }}>
              {/* Оклад */}
              <div style={{ flex: 1, minWidth: '110px', background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '8px', padding: '10px 12px' }}>
                <div style={{ fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.5px', color: 'var(--ink-3)' }}>
                  {salaryType === 'fixed' ? 'Оклад' : `Выходы × ${workDays}`}
                </div>
                <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: '15px', fontWeight: 600, color: 'var(--on-brand)', marginTop: '3px' }}>{fmt(salaryCalc)}</div>
              </div>

              {/* Каждый блок бонусов */}
              {blocksMaxBonuses.map((b, i) => (
                <>
                  <div key={`sep-${i}`} style={{ color: 'var(--chart-tick)', fontSize: '16px' }}>+</div>
                  <div key={`blk-${i}`} style={{ flex: 1, minWidth: '110px', background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '8px', padding: '10px 12px' }}>
                    <div style={{ fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.4px', color: 'var(--ink-3)' }} title={b.name}>{b.name.length > 20 ? b.name.slice(0, 18) + '…' : b.name}</div>
                    <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: '15px', fontWeight: 600, color: 'var(--on-brand)', marginTop: '3px' }}>{fmt(b.max)}</div>
                  </div>
                </>
              ))}

              <div style={{ color: 'var(--ink-4)', fontSize: '16px' }}>+</div>
              <div style={{ flex: 1, minWidth: '100px', background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '8px', padding: '10px 12px' }}>
                <div style={{ fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.5px', color: 'var(--ink-3)' }}>KPI-пункты</div>
                <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: '15px', fontWeight: 600, color: 'var(--on-brand)', marginTop: '3px' }}>{fmt(kpiTotal)}</div>
              </div>

              <div style={{ color: 'var(--ink-4)', fontSize: '16px' }}>=</div>
              <div style={{ flex: 1.5, minWidth: '130px', background: 'var(--brand)', borderRadius: '8px', padding: '10px 14px' }}>
                <div style={{ fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.5px', color: 'var(--line-mid)' }}>Итого</div>
                <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: '20px', fontWeight: 700, color: 'var(--on-brand)', marginTop: '2px' }}>
                  {fmt(ideal)} <span style={{ fontSize: '11px', fontWeight: 400, color: 'var(--line-mid)' }}>сом</span>
                </div>
              </div>
            </div>
          </div>
