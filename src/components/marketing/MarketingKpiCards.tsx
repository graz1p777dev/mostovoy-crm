...
function buildCards(kpi: MarketingKpi): KpiCardDef[] {
  return [
    ...
    },
    {
      ...
      color: kpi.romi >= 500 ? '#15803d' : kpi.romi >= 200 ? '#b45309' : '#e11d1d',
      icon:  'growth',
    ...
    },
    {
      ...
      color: kpi.drr <= 10 ? '#15803d' : kpi.drr <= 20 ? '#b45309' : '#e11d1d',
      icon:  'percent',
      ...
      {cards.map(card => (
        ...
        >
          <div className="flex items-start justify-between mb-2">
            <p className="text-[11px] font-medium leading-tight" style={{ color: '#7d7174' }}>
              {card.label}
            </p>
          ...
          <div className="flex items-center gap-2">
            <p className="text-[11px]" style={{ color: '#7d7174' }}>{card.sub}</p>
            {card.badge && (
              <span
                className="text-[10px] font-semibold px-1.5 py-0.5 rounded"
                style={{
                  backgroundColor: card.badge.good ? '#f0fdf4' : '#fef2f2',
                  color: card.badge.good ? '#15803d' : '#e11d1d',
                }}
              >
                {card.badge.text}
              </span>
