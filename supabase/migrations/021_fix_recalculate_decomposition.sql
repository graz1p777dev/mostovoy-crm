-- Fix: recalculate_decomposition — NULL variables when employee_kpi row missing
-- When SELECT INTO finds no matching row, variables remain NULL (COALESCE inside
-- SELECT only guards column NULLs, not missing-row NULLs). Explicit COALESCE
-- after SELECT INTO ensures NOT NULL INSERT always succeeds.
